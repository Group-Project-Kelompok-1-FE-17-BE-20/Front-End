...
function Cart() {
      ...
      prevItems.map((item) =>
        item.id === itemId
          ? {
              ...
              total_price: Math.max(item.qty - 1, 1) * item.price,
            }
          : item
      )
    );
  ...
  const removeItemFromCart = (itemId: string) => {
    ...
    axios
      .delete(`https://65acaf53adbd5aa31bdf714f.mockapi.io/Keranjang/${itemId}`)
      .then(() => {
  ...
  useEffect(() => {
    const cek = () => {
      axios
        .get<CartState, any>("https://65acaf53adbd5aa31bdf714f.mockapi.io/Keranjang")
        .then((response) => {
  ...
  return (
    <div>
      ...
      <section className="pt-24 w-full mx-auto lg:flex-1 px-[38px] py-[15px] shadow-[0_3px_10px_rgb(0,0,0,0.2)] rounded-md max-w-none lg:max-w-[1404px] mb-10 mt-10">
        ...
        <div className="flex gap-7 w-full mt-10 flex-wrap lg:flex-nowrap">
          <div className="w-full min-w-0 xl:min-w-[715px] space-y-6">
            {cartItems.map(({ id, total_price, qty, model, price, brand, image }) => (
              <div key={id} className="w-full h-[188px] px-6 py-5 bg-white rounded-[20px] shadow-[0_3px_10px_rgb(0,0,0,0.2)] flex-col justify-start items-start gap-6 inline-flex ">
                  ...
                  <div className="grow shrink basis-0 h-[124px] justify-between items-center flex">
                    <div className="h-[118px] flex-col justify-between items-start inline-flex">
                      <div className="flex-col justify-start items-start gap-0.5 flex">
                        <div className="text-zinc-800 text-sm font-medium font-poppins">{brand}</div>
                    ...
                    <div className="w-[225px] h-[124px] flex-col justify-between items-end inline-flex">
                      <div className="h-6 justify-start items-start gap-4 inline-flex">
                        <svg width="24" height="25" viewBox="0 0 24 25" fill="none" xmlns="http://www.w3.org/2000/svg" onClick={() => removeItemFromCart(id)} className="cursor-pointer">
                          <path
