...
function Cart() {
      ...
      prevItems.map((item) =>
        item.product_id === itemId
          ? {
              ...
              total_harga: (item.jumlah + 1) * item.harga_satuan,
            }
          : item
      )
    );
    ...
    setCartItems((prevItems) =>
      prevItems.map((item) =>
        item.product_id === itemId
          ? {
              ...item,
              jumlah: Math.max(item.jumlah - 1, 1),
              total_harga: Math.max(item.jumlah - 1, 1) * item.harga_satuan,
            }
          : item
      )
    );
  ...
  const handleFinalOrder = (itemId: string) => {
    ...
    const product = cartItems.find((item) => item.product_id === itemId);

    // Check if the product is not undefined
    if (product) {
      setFinalOrder((prevOrder) => {
        // Calculate the total price for this product
        const productTotalPrice = product.total_harga;
  ...
  const removeFromFinalOrder = (itemId: string) => {
    setFinalOrder((prevOrder) => {
      const updatedItems = prevOrder.items.filter((item) => item.product_id !== itemId);
      const updatedTotal = updatedItems.reduce((acc, item) => acc + item.total_harga, 0);
  ...
  useEffect(() => {
    const newItemIds = new Set(cartItems.map((item) => item.product_id));
    setCheckedItems(newItemIds);

    // Also update the final order to include all items by default
    setFinalOrder({
      items: [...cartItems],
      total: cartItems.reduce((acc, item) => acc + item.total_harga, 0),
    });
  }, [cartItems]);

  useEffect(() => {
    axios
      .get<CartState>("https://virtserver.swaggerhub.com/Firlydani/Laptop/1.0.0/shopping-cart")
      .then((response) => {
  ...
  return (
    <div>
      ...
      <section className="w-full mx-auto lg:flex-1 px-[38px] py-[15px] shadow-[0_3px_10px_rgb(0,0,0,0.2)] rounded-md max-w-none lg:max-w-[1404px] mb-10 mt-10">
        ...
        <div className="flex gap-7 w-full mt-10 flex-wrap lg:flex-nowrap">
          <div className="w-full min-w-0 xl:min-w-[715px] space-y-6">
            {cartItems.map(({ product_id, total_harga, jumlah, nama_produk, harga_satuan }) => (
                  ...
                  <div className="grow shrink basis-0 h-[124px] justify-between items-center flex">
                    <div className="h-[118px] flex-col justify-between items-start inline-flex">
                      <div className="flex-col justify-start items-start gap-0.5 flex">
                        <div className="text-zinc-800 text-xl font-semibold font-poppins">{nama_produk}</div>
                        <div className="flex-col justify-start items-start gap-1 flex">
                          <div>
                            <span className="text-black text-sm font-normal font-poppins">Harga Satuan : </span>
                            <span className="text-black text-opacity-60 text-sm font-normal font-poppins">Rp {formatToIDR(harga_satuan)}</span>
                          </div>
                        </div>
                      </div>
                      <div className="text-orange-400 text-2xl font-semibold font-poppins">Rp {formatToIDR(total_harga)}</div>
                    </div>
                    <div className="w-[225px] h-[124px] flex-col justify-between items-end inline-flex">
                      <div className="h-6 justify-start items-start gap-4 inline-flex">
                        <svg width="24" height="25" viewBox="0 0 24 25" fill="none" xmlns="http://www.w3.org/2000/svg" onClick={() => removeItemFromCart(product_id)} className="cursor-pointer">
                          <path
                        ...
                        <svg
                          ...
                          height="25"
                          viewBox="0 0 24 25"
                          fill={checkedItems.has(product_id) ? "#0396C7" : "none"}
                          xmlns="http://www.w3.org/2000/svg"
                          onClick={() => toggleCheckbox(product_id)}
                      ...
                      <div className="px-5 py-3 bg-zinc-100 rounded-lg justify-center items-center gap-5 inline-flex">
                        <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" onClick={() => decrementQuantity(product_id)} className="cursor-pointer">
                          <path
                        ...
                        <div className="text-black text-sm font-medium font-poppins">{jumlah}</div>
                        <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" onClick={() => incrementQuantity(product_id)} className="cursor-pointer">
                          <path
          ...
          <div className="w-full min-w-0 xl:min-w-[505px] px-6 py-5 bg-white rounded-[20px] shadow-[0_3px_10px_rgb(0,0,0,0.2)] flex-col justify-start items-start gap-6 inline-flex h-min">
            ...
            <div className="self-stretch  flex-col justify-start items-start gap-5 flex">
              {finalOrder.items.map(({ product_id, total_harga, nama_produk }) => (
                <div key={product_id} className="self-stretch justify-between items-center inline-flex">
                  <div className="text-black text-opacity-60 text-xl font-normal font-poppins">{nama_produk}</div>
                  <div className="text-right text-zinc-800 text-xl font-semibold font-poppins">Rp {formatToIDR(total_harga)}</div>
                </div>
              ))}
