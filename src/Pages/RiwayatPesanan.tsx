...
import { OrderHistoryType } from "../utils/interface";

function RiwayatPesanan(props: any) {
    ...
    <div>
      ...
      <div>
        <div className={!hidden ? `px-3 md:px-10 pt-7 w-full` : `px-3 md:px-24 pt-7`}>
          <h2 className="font-poppins text-3xl font-semibold leading-4 mb-4">Riwayat Pesanan Saya</h2>
            ...
            <ol className="inline-flex items-center space-x-1 md:space-x-3">
              <li className="inline-flex items-center">
                <a href="#" className="inline-flex items-center text-xl font-medium text-gray-500 hover:text-gray-700">
                  Lihat riwayat pesanan anda yang sudah selesai ataupun yang sedang berlangsung
                </a>
              </li>
            </ol>
          </nav>
...
function OrderHistory() {
  ...
  useEffect(() => {
    axios
      .get<OrderHistoryType[]>("https://virtserver.swaggerhub.com/Firlydani/Laptop/1.0.0/order-history/string")
      .then((response: AxiosResponse<OrderHistoryType[]>) => {
    ...
    <div className="w-full">
      ...
      <section className="w-full lg:flex-1 px-4 py-4 rounded-md mb-8 shadow-sm space-y-4 max-h-screen overflow-auto">
        {orderHistory.map(({ order_id, item_produk, status }) => (
          <div key={order_id} className="w-full h-auto relative bg-sky-50 rounded-lg p-4 font-poppins">
            <div className="flex justify-between items-center pb-4 border-b-2 border-[#E6E6E6]">
              <span className="flex items-center gap-4">
                <div className="text-zinc-800 text-base font-medium">STORE ID</div>
                <button className="mt-2 px-3 py-1 rounded-lg border border-sky-600 text-sky-600 text-xs font-medium">Kunjungi Toko</button>
              </span>
              <p className="text-right text-sky-600 text-base font-medium font-poppins">{status}</p>
            </div>
            <div className="border-b-2 border-[#E6E6E6]">
              {item_produk.map(({ product_id, nama_produk, jumlah, total_harga, harga_satuan }) => (
                <div key={product_id} className="w-full flex justify-between items-center py-5">
                  <div className="flex items-center gap-4 w-full">
                    <img className="w-24 h-36 object-cover" src="https://via.placeholder.com/98x146" alt="Product" />
                    <div className="flex flex-col justify-start items-start gap-2 w-full">
                      <div className="text-zinc-800 text-lg font-semibold">{nama_produk}</div>
                      <div className="flex flex-col justify-start items-start gap-1">
                        <div className="text-black text-sm">
                          Harga Satuan : <span className="text-opacity-60">{formatToIDR(harga_satuan)}</span>
                        </div>
                      </div>
                      <div className="flex items-center pb-3 justify-between w-full">
                        <div className="text-sky-600 text-lg font-semibold">x {jumlah === 0 ? 1 : jumlah}</div>
                        <div className="flex justify-between gap-5 items-center mt-4">
                          <div className="text-neutral-400 text-base font-medium">Total Pesanan</div>
                          <div className="text-sky-600 text-2xl font-semibold">Rp {formatToIDR(total_harga)}</div>
                        </div>
                      </div>
                    </div>
                  </div>
            ...
            <div className="flex justify-end pt-3">
              <div className="w-80 h-10 py-2.5 bg-sky-600 rounded-md justify-start items-start gap-2.5 inline-flex">
                <div className="w-80 h-5 text-center text-white text-sm font-medium font-['Poppins']">Pesanan Selesai</div>
              </div>
            </div>
          </div>
        ))}
