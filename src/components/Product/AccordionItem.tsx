...
const AccordionItem: FC<AccordionItemProps> = ({ title, children, alwaysOpen = false, sub, required = false }) => {
  ...
  return (
    <div className="border-b-2 py-4">
      <div className="flex">
        <button className="py-2 px-4 w-full text-left font-poppins font-semibold text-base" onClick={() => !alwaysOpen && setIsOpen(!isOpen)} disabled={alwaysOpen} type="button">
          <h4 className="text-[#111827]">
            {title}
            {required && <span className="text-[#F43F5E]">*</span>}
          </h4>
          {sub && <p className="text-sm font-normal text-[#6B7280]">{sub}</p>}
        </button>
        {isOpen || alwaysOpen ? (
      ...
      {(isOpen || alwaysOpen) && <div className="p-4">{children}</div>}
    </div>
  );
};
